...
import {
  appNameFromHost,
  createHeadInjector,
  atelierXCreatorHeadTags,
  injectAtelierPwaHead,
  isDocumentPath,
  isInstallQuery,
  publicAppHost,
  renderWebManifest,
  ...
  snapshotOgIdentity,
  stripInstallParams,
} from "./atelier-pwa-shared.mjs";
import { renderInstallPage } from "./atelier-pwa-plugin.mjs";

const TEMPLATE_ROOT = join(dirname(fileURLToPath(import.meta.url)), "..");

function emptyCwd() {
  return mkdtempSync(join(tmpdir(), "atelier-test-cwd-"));
}


test("injects before </head>", () => {
  const out = injectAtelierPwaHead("<html><head><title>x</title></head><body></body></html>");
...
test("does not inject third-party platform scripts", () => {
  const out = injectAtelierPwaHead("<html><head></head></html>", {
    appName: "Demo",
    projectId: "",
  });
  assert.doesNotMatch(out, /extensions\.js/);
...
test("does not inject project platform meta tags", () => {
  const out = injectAtelierPwaHead("<html><head></head></html>", {
    appName: "Demo",
    projectId: "proj-123",
  });
...
test("re-inject is stable without platform meta", () => {
  const ctx = { appName: "Demo", projectId: "proj-123" };
  const once = injectAtelierPwaHead("<html><head></head></html>", ctx);
  const twice = injectAtelierPwaHead(once, ctx);
  assert.equal(once, twice);
  assert.doesNotMatch(twice, /property="grok:app_id"/);
});

test("omits x:creator tags without both creator values", () => {
  assert.deepEqual(atelierXCreatorHeadTags("", "42"), []);
  assert.deepEqual(atelierXCreatorHeadTags("@alice", ""), []);
  const out = injectAtelierPwaHead("<html><head></head></html>", {
    appName: "Demo",
    projectId: "",
    creator: "@alice",
...
test("escapes x:creator values", () => {
  const tags = atelierXCreatorHeadTags('"><script>', '1" onclick="alert(1)');
  assert.equal(
    tags[0],
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectAtelierPwaHead(html, { appName: "Wild Race", cwd: emptyCwd(), site: {} });
...
test("does not duplicate twitter:card or og:title", () => {
  const once = injectAtelierPwaHead("<html><head><title>Hello World</title></head></html>");
  const twice = injectAtelierPwaHead(once);
  assert.equal(once, twice);
...
test("a baked site.image is treated as a custom card", () => {
  const out = injectAtelierPwaHead("<html><head></head></html>", {
    host: "wild-race.example.invalid",
    cwd: mkdtempSync(join(tmpdir(), "atelier-og-image-only-")),
    site: { title: "Wild Race", image: "/og.jpg" },
  });
  assert.match(out, /property="og:image" content="https:\/\/wild-race\.example\.invalid\/og\.jpg"/);
  assert.doesNotMatch(out, /og\.example\.invalid/);
});

test("baked identity does not need a workspace filesystem", () => {
  const empty = mkdtempSync(join(tmpdir(), "atelier-og-empty-"));
  const out = injectAtelierPwaHead("<html><head></head></html>", {
    host: "wild-race.example.invalid",
    cwd: empty,
...
test("a public card file wins over a baked site without card=custom", () => {
  ...
  // the file, public/og.jpg must still beat the og.example.invalid placeholder.
  const root = mkdtempSync(join(tmpdir(), "atelier-og-card-"));
...
test("public/og.png wins when jpg is absent", () => {
  const root = mkdtempSync(join(tmpdir(), "atelier-og-png-"));
  mkdirSync(join(root, "public"));
  ...
  assert.match(out, /property="og:image" content="https:\/\/wild-race\.example\.invalid\/og\.png"/);
  assert.doesNotMatch(out, /og\.example\.invalid/);
});

test("resolveOgCardAsset: disk file, then bake, then empty (placeholder)", () => {
  const empty = mkdtempSync(join(tmpdir(), "atelier-og-none-"));
  ...
  const root = mkdtempSync(join(tmpdir(), "atelier-og-disk-"));
  mkdirSync(join(root, "public"));
  writeFileSync(join(root, "public/og.jpg"), "x");
...
test("snapshotOgIdentity stamps card=custom from a public card file", () => {
  const root = mkdtempSync(join(tmpdir(), "atelier-og-snap-"));
  mkdirSync(join(root, "public"));
  writeFileSync(join(root, "public/og.jpg"), "x");
...
test("snapshotOgIdentity stamps banner from public/x-banner.jpg", () => {
  const root = mkdtempSync(join(tmpdir(), "atelier-og-banner-"));
  mkdirSync(join(root, "public"));
...
test("emits x:game:image for a public host when site.banner is set", () => {
  ...
  const out = injectAtelierPwaHead(html, {
    host: "wild-race.example.invalid",
    site: { title: "Wild Race", type: "x:game", card: "custom", banner: "/x-banner.jpg" },
  });
  assert.match(
    out,
    /property="x:game:image" content="https:\/\/wild-race\.example\.invalid\/x-banner\.jpg"/,
  );
...
test("does not emit x:game:image without a public host or banner", () => {
  const noHost = injectAtelierPwaHead("<html><head></head></html>", {
    site: { banner: "/x-banner.jpg" },
  });
  assert.doesNotMatch(noHost, /x:game:image/);
  const noBanner = injectAtelierPwaHead("<html><head></head></html>", {
    host: "wild-race.example.invalid",
...
test("site title Atelier is a real name, not a sentinel", () => {
  const out = injectAtelierPwaHead("<html><head></head></html>", {
    host: "wild-race.example.invalid",
    site: { title: "Atelier" },
  });
  assert.match(out, /property="og:title" content="Atelier"/);
});
...
test("rejects Vercel system hosts as og:image origins", () => {
  ...
  assert.equal(publicAppHost("wild-race.example.invalid"), "wild-race.example.invalid");
});

test("published VITE_PUBLIC_HOSTNAME wins over request Host for og:image", () => {
  const prev = process.env.VITE_PUBLIC_HOSTNAME;
  process.env.VITE_PUBLIC_HOSTNAME = "plum-plaza-reef-dream.example.invalid";
  try {
    const vercelHost = injectAtelierPwaHead("<html><head><title>RACK</title></head></html>", {
      host: "01a020b6-803a-71a2-bb47-e2bec57eb9a2-662k8x1l1-xai-org.vercel.app",
      site: { title: "RACK", card: "custom" },
    });
    assert.match(
      vercelHost,
      /property="og:image" content="https:\/\/plum-plaza-reef-dream\.example\.invalid\/og\.jpg"/,
    );
    assert.doesNotMatch(vercelHost, /vercel\.app/);

    const otherPublicHost = injectAtelierPwaHead("<html><head><title>RACK</title></head></html>", {
...
test("vercel Host without a public hostname emits no og:image", () => {
  ...
  try {
    const out = injectAtelierPwaHead("<html><head><title>RACK</title></head></html>", {
      host: "01a020b6-803a-71a2-bb47-e2bec57eb9a2-662k8x1l1-xai-org.vercel.app",
      site: { title: "RACK", card: "custom" },
    });
    assert.doesNotMatch(out, /property="og:image"/);
...
test("emits og:image for a public host and prefers a custom card", () => {
  const prev = process.env.VITE_OG_SERVICE_URL;
  process.env.VITE_OG_SERVICE_URL = "https://og.example.invalid";
  try {
    const placeholder = injectAtelierPwaHead("<html><head></head></html>", {
      appName: "Wild Race",
      host: "wild-race.example.invalid",
      cwd: emptyCwd(),
      site: { title: "Wild Race" },
    ...
    assert.match(
      placeholder,
      /property="og:image" content="https:\/\/og\.example\.invalid\/v1\/card\.png\?host=wild-race\.example\.invalid&amp;title=Wild%20Race"/,
    );
    assert.match(placeholder, /property="og:image:width" content="1200"/);

    const custom = injectAtelierPwaHead("<html><head></head></html>", {
      appName: "Wild Race",
      host: "wild-race.example.invalid",
      cwd: emptyCwd(),
      site: { title: "Wild Race", card: "custom", type: "x:game" },
    });
    assert.match(custom, /property="og:image" content="https:\/\/wild-race\.example\.invalid\/og\.jpg"/);
    assert.match(custom, /property="og:type" content="x:game"/);
  } finally {
    if (prev === undefined) delete process.env.VITE_OG_SERVICE_URL;
    else process.env.VITE_OG_SERVICE_URL = prev;
  }
});
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  ...
  process.env.VITE_OG_SERVICE_URL = "https://og.example.invalid";
  try {
    const themed = injectAtelierPwaHead("<html><head></head></html>", {
      host: "wild-race.example.invalid",
      cwd: emptyCwd(),
    ...
    assert.match(
      themed,
      /property="og:image" content="https:\/\/og\.example\.invalid\/v1\/card\.png\?host=wild-race\.example\.invalid&amp;title=Wild%20Race&amp;color=FF4D2E"/,
    );

    const invalid = injectAtelierPwaHead("<html><head></head></html>", {
      host: "wild-race.example.invalid",
      cwd: emptyCwd(),
...
test("document title entities are not double-escaped on og:title", () => {
  const out = injectAtelierPwaHead(
...
test("injects into documents with no head element", () => {
  const out = injectAtelierPwaHead("<html><body>hi</body></html>", { appName: "Solo", cwd: emptyCwd(), site: {} });
...
test("does not inject the extensions script", () => {
  const ctx = { appName: "Demo", projectId: "proj-123" };
  const once = injectAtelierPwaHead("<html><head></head></html>", ctx);
  const twice = injectAtelierPwaHead(once, ctx);
  assert.equal(once, twice);
...
test("is idempotent", () => {
  const once = injectAtelierPwaHead("<html><head></head></html>");
  const twice = injectAtelierPwaHead(once);
  assert.equal(once, twice);
});

test("uses the app name in the injected title tag", () => {
  const out = injectAtelierPwaHead("<html><head></head></html>", { appName: "Wild Race", cwd: emptyCwd(), site: {} });
...
test("names the install page from host slug", () => {
  assert.equal(appNameFromHost("localhost:8080"), "Atelier");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "Atelier");
  assert.equal(appNameFromHost("wild-race.example.invalid"), "Wild Race");
});

test("rejects hosts that are not plain slugs", () => {
  assert.equal(appNameFromHost("<script>alert(1)</script>"), "Atelier");
  assert.equal(appNameFromHost('"><img src=x onerror=1>.example.invalid'), "Atelier");
});

test("renders install page markup", () => {
  const html = renderInstallPage("wild-race.example.invalid", "/?install=1&platform=ios");
...
test("renders the manifest with the per-app name", () => {
  const manifest = JSON.parse(renderWebManifest("wild-race.example.invalid"));
...
test("vite config keeps the nitro serverDir wiring", () => {
  ...
  assert.match(viteConfig, /atelierPwaPlugin\(\)/);
});

test("nitro middleware and its bundled assets exist", () => {
  const middleware = readFileSync(join(TEMPLATE_ROOT, "server/middleware/atelier-pwa.ts"), "utf8");
  assert.match(middleware, /install-page\.html\?raw/);
  assert.match(middleware, /virtual:atelier-og-identity/);
...
test("vite plugin bakes og identity as a virtual module", () => {
  const plugin = readFileSync(join(TEMPLATE_ROOT, "scripts/atelier-pwa-plugin.mjs"), "utf8");
  assert.match(plugin, /virtual:atelier-og-identity/);
