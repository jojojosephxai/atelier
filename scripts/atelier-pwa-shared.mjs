...
export const DEFAULT_APP_NAME = "Atelier";
...
/** 6-digit hex for the og.example.invalid placeholder, or "" if site.color is missing/invalid. */
...
/**
 * "wild-race.example.invalid" → "Wild Race". Only published app hosts encode the
...
export function atelierPwaHeadTags(appName = DEFAULT_APP_NAME) {
  return [
...
export const ATELIER_EXTENSIONS_SCRIPT_SRC = "";

export function readAtelierProjectId() {
...
export function atelierXCreatorHeadTags(creator = readXCreator(), creatorId = readXCreatorId()) {
...
/** No third-party platform banner — Atelier is standalone. */
export function atelierExtensionsHeadTags(_projectId = readAtelierProjectId()) {
  return [];
}
 ...
 * Otherwise empty — caller emits the og.example.invalid placeholder.
 */
...
export function atelierOgHeadTags({
  host = "",
  appName = DEFAULT_APP_NAME,
  site = {},
  documentTitle = "",
...
export function normalizeHeadContext(ctx = {}) {
  ...
  // wins over the og.example.invalid placeholder. Vercel has no public/ to read, so
  ...
  return {
    appName,
    projectId: ctx.projectId ?? readAtelierProjectId(),
...
export function injectAtelierPwaHead(html, ctx = {}) {
  ...
  const missing = atelierPwaHeadTags(appName)
    .filter(([key]) => {
  ...
  next = insertAfterHeadOpen(
    next,
    atelierOgHeadTags({ host, appName, site, documentTitle, cwd }).join(""),
  );

  missing.push(...atelierExtensionsHeadTags(projectId));
  const creatorTags = atelierXCreatorHeadTags(creator, creatorId);
