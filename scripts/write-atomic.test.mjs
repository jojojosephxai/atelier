...
test("parseWriteAtomicArgs needs exactly a staged file and a target", () => {
  assert.deepEqual(parseWriteAtomicArgs([".atelier/og.tmp", "public/og.jpg"]), {
    staged: ".atelier/og.tmp",
    target: "public/og.jpg",
  });
  assert.match(parseWriteAtomicArgs([]).error, /usage:/);
  assert.match(parseWriteAtomicArgs([".atelier/og.tmp"]).error, /usage:/);
...
test("stagingError refuses a temp inside public/ and a no-op move", () => {
  ...
  assert.equal(
    stagingError({
      staged: "/workspace/.atelier/og.jpg.tmp",
...
test("handOver replaces the target and clears the staged file", () => {
  ...
  const staged = join(root, ".atelier/og.jpg.tmp");
...
test("handOver creates a missing target directory", () => {
  const root = makeWorkspace();
  const staged = join(root, ".atelier/site.json.tmp");
...
test("an interrupted pass leaves the target on its old bytes, temp-free", () => {
  ...
  writeFileSync(join(root, ".atelier/og.jpg.tmp"), "half a JPEG");

  assert.equal(readFileSync(target, "utf8"), "old card");
  assert.throws(() => handOver(join(root, ".atelier/absent.tmp"), target), { code: "ENOENT" });
  assert.equal(readFileSync(target, "utf8"), "old card");
...
test("a failed hand-over creates no directory for the target it never wrote", () => {
  const root = makeWorkspace();
  const missing = join(root, ".atelier/absent.tmp");
...
test("a staged file on another filesystem is refused, not copied", () => {
  ...
  assert.throws(() => handOver(staged, target, { rename: crossDevice }), {
    message: /stage under \/workspace\/\.atelier\//,
  });
...
test("cli: hands the file over, and refuses a temp staged in public/", () => {
  const root = makeWorkspace();
  writeFileSync(join(root, ".atelier/og.jpg.tmp"), "new card");
  const ok = spawnSync(
    process.execPath,
    [SCRIPT, join(root, ".atelier/og.jpg.tmp"), join(root, "public/og.jpg")],
    { encoding: "utf8" },
  );
...
test("every hand-over the og skill prints is one this script accepts", { skip: "platform agent docs removed" }, () => {
  // The card and banner recipes live in the skill's references/, not SKILL.md.
  const skillDir = join(TEMPLATE_ROOT, ".atelier/skills/og");
  const docs = [
...
test("cli: a missing staged file fails without touching the target", () => {
  ...
  const run = spawnSync(
    process.execPath,
    [SCRIPT, join(root, ".atelier/absent.tmp"), join(root, "public/og.jpg")],
    { encoding: "utf8" },
  );
