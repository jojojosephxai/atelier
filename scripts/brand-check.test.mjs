...
test("SKILL.md and AGENTS.md name the marker path and bound this script uses", { skip: "platform agent docs removed" }, () => {
  // Prose wraps, so the minute count may straddle a line break.
  const bound = new RegExp(`${OG_PENDING_MAX_AGE_MS / 60_000}\\s+minutes`);
  for (const rel of [".atelier/skills/og/SKILL.md", "AGENTS.md"]) {
...
const PROHIBITION_SECTIONS = [
  {
    rel: ".atelier/skills/og/SKILL.md",
...
test("the sections that own the brand-task prohibition never affirm a wait", { skip: "platform agent docs removed" }, () => {
...
test("SKILL.md tells the pass to self-check with the flag this CLI accepts", { skip: "platform agent docs removed" }, () => {
  const skill = readDoc(".atelier/skills/og/SKILL.md");
