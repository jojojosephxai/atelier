 ...
 * those must ship a custom share card — the default og.example.invalid placeholder is
 * not acceptable for them (see .atelier/skills/og/SKILL.md).
 *
 ...
 * favicon, PWA icons and title for a plain utility that keeps the og.example.invalid
 * card — where no card is the expected verdict rather than a failure.
 */
import { existsSync, statSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath, pathToFileURL } from "node:url";
import { OG_SITE_REL_PATH, readOgSite, siteHasCustomCard } from "./atelier-pwa-shared.mjs";
...
export const OG_PENDING_REL_PATH = ".atelier/og-pending";
...
function brandWarningsOnDisk({
  ...
  const skillPath = join(workspaceRoot, ".atelier/skills/og/SKILL.md");
  ...
  if (cardPath !== undefined) {
    ...
    warnings.push(
      `BRAND WARNING: this looks like a game/canvas app but ${workspaceRoot}/public/og.jpg `
        ...
        + "built from the app's own art — the default og.example.invalid placeholder card is not "
    ...
    warnings.push(
      `BRAND WARNING: ${workspaceRoot}/public/og.jpg is missing and this pass exists to `
        ...
        + "report that instead of reporting a pass — the app keeps the og.example.invalid "
        + "placeholder.",
    );
  } else {
    warnings.push(
      "BRAND NOTE: no custom public/og.jpg — the platform will serve the og.example.invalid placeholder. "
  ...
  // Skip while still on the og.example.invalid placeholder — that pass has not started yet.
