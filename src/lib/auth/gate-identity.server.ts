...
export const GATE_IDENTITY_HEADER = "x-atelier-identity";
...
const FALLBACK_EMAIL_DOMAIN = "viewer.atelier.invalid";
const FALLBACK_NAME = "Atelier user";
...
export function gateTokenAudience(): string {
  const projectId = env("ATELIER_PROJECT_ID");
...
export function resolveGateEndpoints(headers: Headers): GateEndpoints | null {
  ...
  if (
    host === "atelier-testing.example" ||
    host.endsWith(".atelier-testing.example")
  ) {
    issuer = "https://gate.atelier-testing.example";
  } else if (host === "atelier.local" || host.endsWith(".example.invalid")) {
    issuer = "https://gate.example.invalid";
  }
