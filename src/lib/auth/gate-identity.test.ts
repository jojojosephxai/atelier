...
const ISSUER = "https://gate.atelier-testing.example";
...
describe("gateIdentityFromHeaders", () => {
  it("verifies the header token end to end and fails closed without it", async () => {
    ...
    process.env.ATELIER_PROJECT_ID = "proj-123";
    process.env.ATELIER_GATE_ORIGIN = ISSUER;
    try {
      const token = await signToken(key, {
        sub: "user-1",
        email: "viewer@example.com",
      ...
      const withToken = await gateIdentityFromHeaders(
        new Headers({ "x-atelier-identity": token }),
        fetchImpl,
      );
      ...
      delete process.env.ATELIER_PROJECT_ID;
      delete process.env.ATELIER_GATE_ORIGIN;
    }
  });

  it("activates on a deployed-shaped request without ATELIER_GATE_ORIGIN", async () => {
    ...
    try {
      ...
      const identity = await gateIdentityFromHeaders(
        new Headers({
          host: "my-app.atelier-testing.example",
          "x-atelier-identity": token,
        }),
        fetchImpl,
      );
      ...
      assert.equal(
        fetchedFrom[0],
        "https://gate.atelier-testing.example/__gate/identity-key",
      );
    } finally {
      delete process.env.ATELIER_PROJECT_ID;
    }
  ...
  it("verifies a preview-audience token with no gate env vars via the loopback default", async () => {
    ...
    const identity = await gateIdentityFromHeaders(
      new Headers({
        host: "my-session.atelier-preview.local",
        "x-atelier-identity": token,
      }),
      fetchImpl,
    );
  ...
  it("verifies a preview-audience token when only ATELIER_GATE_ORIGIN is set", async () => {
    const key = await makeKey("k1");
    const { fetchImpl } = staticJwks([key.jwk]);
    delete process.env.ATELIER_PROJECT_ID;
    process.env.ATELIER_GATE_ORIGIN = ISSUER;
  ...
  it("rejects a preview-audience token when ATELIER_PROJECT_ID is set", async () => {
    const key = await makeKey("k1");
    const { fetchImpl } = staticJwks([key.jwk]);
    process.env.ATELIER_PROJECT_ID = "proj-123";
    process.env.ATELIER_GATE_ORIGIN = ISSUER;
...
describe("gateTokenAudience", () => {
  it("pins app:<id> when ATELIER_PROJECT_ID is set, even alongside ATELIER_GATE_ORIGIN", () => {
    process.env.ATELIER_PROJECT_ID = "proj-123";
    process.env.ATELIER_GATE_ORIGIN = ISSUER;
    try {
  ...
  it("pins preview when ATELIER_PROJECT_ID is unset", () => {
    delete process.env.ATELIER_PROJECT_ID;
    process.env.ATELIER_GATE_ORIGIN = ISSUER;
    try {
...
describe("gateIdentityUserInfo", () => {
  it("falls back to a synthetic email and name for sub-only claims", () => {
    assert.deepEqual(
      ...
      {
        id: "User-1",
        email: "user-1@viewer.atelier.invalid",
        emailVerified: false,
        name: "Atelier user",
      },
    );
