 ...
 * (`ATELIER_AUTH_ISSUER`) via the `genericOAuth` plugin — the broker brokers the
 ...
 *
 * Tri-mode:
 *   - Deployed: the deployer injects a per-app `ATELIER_AUTH_*` + `BETTER_AUTH_URL`
 *     + `DATABASE_URL`, so real federated auth is persisted in Postgres.
 *   - Sandbox live preview: no injection -> falls back to the shared **preview
 *     client** (`./preview`) and derives the preview's `https://*.atelier-preview.local`
...
import { ATELIER_AUTH_PROVIDERS } from "./providers";
...
const globalAuthRef = globalThis as typeof globalThis & {
  __atelierAuthPreviewSecret__?: string;
};
function previewAuthSecret(): string {
  globalAuthRef.__atelierAuthPreviewSecret__ ??= randomBytes(32).toString("hex");
  return globalAuthRef.__atelierAuthPreviewSecret__;
}
...
// for any `*.atelier-preview.local` callback (see `./preview`).
const authIssuer = env("ATELIER_AUTH_ISSUER") ?? AUTH_ISSUER_DEFAULT;
const authClientId = env("ATELIER_AUTH_CLIENT_ID") ?? PREVIEW_CLIENT_ID;
const authClientSecret = env("ATELIER_AUTH_CLIENT_SECRET") ?? PREVIEW_CLIENT_SECRET;
...
// a dynamic `*.atelier-preview.local` host), so we hand Better Auth a dynamic baseURL:
...
export const SESSION_TOKEN_COOKIE = "__Host-atelier-auth.session_token";

// Built separately so the `betterAuth({...})` call stays easy to edit without
// breaking brackets (models often trip on the conditional plugin spread).
const authOAuthPlugin = authConfigured
  ? genericOAuth({
      config: ATELIER_AUTH_PROVIDERS.map(({ providerId, idp }) => ({
        providerId,
...
export const auth = betterAuth({
  ...
  account: {
    ...
    accountLinking: {
      enabled: true,
      trustedProviders: [
        ...ATELIER_AUTH_PROVIDERS.map((p) => p.providerId),
        GATE_PROVIDER_ID,
      ],
  ...
  // carries a `Domain` attribute, so a sibling `*.example.invalid` app cannot "toss" a
  // `Domain=.example.invalid` session cookie onto this app. `__Host-` requires Secure +
  ...
  advanced: {
    ...
    cookies: {
      ...
      session_data: { name: "__Host-atelier-auth.session_data" },
      account_data: { name: "__Host-atelier-auth.account_data" },
      dont_remember: { name: "__Host-atelier-auth.dont_remember" },
    },
  },
...
export { ATELIER_AUTH_PROVIDERS } from "./providers";
