...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  try {
    const apiRes = await auth.api.signInWithOAuth2({
      ...
      // redirect_uri for the dynamic `*.atelier-preview.local` origin.
...
<script type="application/json" id="atelier-auth-popup-msg">${payload}</script>
<script>
(function () {
  var el = document.getElementById("atelier-auth-popup-msg");
  var msg = { source: "atelier-auth-popup", token: null };
