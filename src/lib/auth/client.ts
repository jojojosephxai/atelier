 ...
 * (`.atelier/app-env.json`), which selects the dev user (see `use-current-user`);
...
const BEARER_KEY = "atelier-auth.bearer-token";
...
/**
 * The sandbox live preview runs this app inside an iframe on a `*.atelier-preview.local`
...
function inLivePreview(): boolean {
  return (
    ...
    window.location.hostname.endsWith(".atelier-preview.local")
  );
}

/** Message the popup posts back to the opener once sign-in completes. */
type PopupMessage = { source: "atelier-auth-popup"; token: string | null; error?: string };

/**
 * Start sign-in with one upstream provider (`providerId` from `ATELIER_AUTH_PROVIDERS`),
 * federating through the auth broker.
 *
 * - **Live preview** (`*.atelier-preview.local` iframe): opens a POPUP to
...
function openSignInPopup(providerId: string): Window | null {
  ...
  const name = `atelier-signin-${Date.now()}`;
...
function waitForPopupToken(popup: Window): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.source !== "atelier-auth-popup") return;
