...
export const CONNECTORS_HOST_STAGING = "connectors.atelier-testing.example";
export const CONNECTORS_HOST_PROD = "connectors.example.invalid";
...
function connectorsBaseFor(publicHost: string | null): string | null {
  const explicit = env("ATELIER_CONNECTORS_URL");
  ...
  if (host === "atelier.local" || host.endsWith(".example.invalid")) {
...
function inboundContext(): InboundContext {
  ...
  const envToken =
    process.env.NODE_ENV === "production"
      ? null
      : (env("ATELIER_CONNECTOR_ACCESS_TOKEN") ?? null);
  return {
