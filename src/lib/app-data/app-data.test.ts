...
async function withStubbedGate(
  ...
  process.env.ATELIER_CONNECTORS_URL = "https://connectors.invalid.example";
  ...
  try {
    ...
    delete process.env.ATELIER_CONNECTORS_URL;
  }
}
...
describe("redirectToLoginIfRequired", () => {
    ...
    const did = withWindow(
      {
        location: {
          ...
          href: "https://my-app.example.invalid/current",
        },
      },
      () =>
    ...
    const did = withWindow(
      ...
      () =>
        redirectToLoginIfRequired({
          ...
          data: null,
          loginRequired: true,
          loginUrl: "https://gate.example.invalid/__gate/signin?return_to=x",
        }),
    );
    assert.equal(did, true);
    assert.equal(target, "https://gate.example.invalid/__gate/signin?return_to=x");
  });
  ...
  it("opens a new tab instead of navigating when framed", () => {
    ...
    assert.equal(did, true);
    assert.equal(opened, "https://gate.example.invalid/__gate/signin?return_to=x");
  ...
  it("falls back to navigation when framed and the popup is blocked", () => {
    ...
    assert.equal(did, true);
    assert.equal(assigned, "https://gate.example.invalid/__gate/signin?return_to=x");
  });
