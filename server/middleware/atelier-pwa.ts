 ...
 *   OG identity is baked via `virtual:atelier-og-identity` at `vite build`
...
import { atelierOgIdentity } from "virtual:atelier-og-identity";
import {
  acceptsHtml,
  createHeadInjector,
  isDocumentPath,
  ...
  renderWebManifest,
} from "../../scripts/atelier-pwa-shared.mjs";

interface AtelierPwaEvent {
  url: URL;
  req: { method: string; headers: Headers };
}

function requestHost(event: AtelierPwaEvent): string {
  return (
...
export default async function atelierPwaMiddleware(
  event: AtelierPwaEvent,
